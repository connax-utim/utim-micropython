...
def long_to_bytes(n):
    ...
    while x != n:
        byte = (n >> off) & 0xFF
        hexbyte = binascii.unhexlify("{0:0>2}".format(str(hex(byte))[2:]))
...
def H(hash_class, *args, **kwargs):
    ...
    for s in args:
        if s is not None:
            h.update(long_to_bytes(s) if isinstance(s, int) else s)
...
class Verifier(object):
    def __init__(self, username, bytes_s, bytes_v, bytes_A, hash_alg=SHA256, ng_type=NG_1024,
        ...
        self.salt = bytes_to_long(bytes_s)
        ...
        # SRP-6a safety check
        self.saltafety_failed = self.A % N == 0

        if not self.saltafety_failed:
            ...
            self.M = calculate_M(hash_class, N, g, self.identity, self.salt, self.A, self.B, self.K)
    ...
    def get_challenge(self):
        if self.saltafety_failed:
            return None, None
        else:
            return (long_to_bytes(self.salt), long_to_bytes(self.B))

    # returns H_AMK on success, None on failure
    def verify_session(self, user_M):
        if not self.saltafety_failed and user_M == self.M:
...
class User(object):
    ...
    def process_challenge(self, bytes_s, bytes_B):
        ...
        self.x = gen_x(hash_class, self.salt, self.identity, self.password)
